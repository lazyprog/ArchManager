...
Процедура НачатьУправление(АдресФайлаНастроек)
	...
	Показатели = ПолучитьКлючевыеПоказатели(ТаблицаФайлов);
	
КонецПроцедуры

Функция ПолучитьКлючевыеПоказатели(ТаблицаДанных)
	
	КлючевыеПоказатели = Новый Структура();
	...
	КлючевыеПоказатели.Вставить("КоличествоФайлов", ТаблицаДанных.Количество());
	
	// Общий занимаемый объем в Мегабайтах
	ОбщийОбъем = ТаблицаДанных.Итог("Размер");
	КлючевыеПоказатели.Вставить("ОбщийОбъем", Окр((ОбщийОбъем / 1024) / 1024, 3));
	
	// Свободное место на диске
	ПутьКаталога = ТаблицаДанных[0].Путь;
	ИмяДиска = ПолучитьВложенность(ПутьКаталога, "")[0];
	ДискиСистемы = Новый Име

	СИ = Новый СистемнаяИнформация();
	СИ.ИменаЛогическихДисков; 
...
Функция ПолучитьВложенность(СтрокаПуть, СтрокаКорень)
	Если СтрДлина(СтрокаКорень) > 0 Тогда
		ПутьДляРазбора = СтрЗаменить(СтрокаПуть, СтрокаКорень, "");
	Иначе
		ПутьДляРазбора = СтрокаПуть;
	КонецЕсли;
